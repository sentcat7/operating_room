...
class CombinedDetector:
    ...
    def test_image(self, input_path, output_path="results/combined_image.jpg"):
        frame = cv2.imread(input_path)
        if frame is None:
            raise FileNotFoundError(f"无法读取图片: {input_path}")

        processed_frame = self.run(frame)
    ...
    def test_video(self, input_path, output_path=None):
...
if __name__ == "__main__":
    ...
    # 输入输出配置（请根据实际情况修改）
    # INPUT_SOURCE = "/home/yangf/algo_yolo/algo_all/yu_pose_cloth/algos/a9185d86-MVI_8621_11250.jpg"
    ...
    if INPUT_SOURCE.lower().endswith(('.jpg', '.jpeg', '.png')):
        detector.test_image(INPUT_SOURCE, OUTPUT_PATH)
    else:
        detector.test_video(INPUT_SOURCE, OUTPUT_PATH)
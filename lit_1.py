...
# 全局变量用于记录已经加载的模型
loaded_models = {}
# 用于记录每个人物任意一只手低于腰满足条件的时间戳
person_any_hand_low_waist_timestamps = {}
# 用于记录每个人物任意一只手高于头满足条件的时间戳
person_any_hand_high_head_timestamps = {}
...
def output(app: StreamlitApp, name, position, data):
    ...
    global algo_containers, algo_states, person_any_hand_low_waist_timestamps, person_any_hand_high_head_timestamps
    ...
    if name == "CombinedDetector":
        ...
        # 只处理前两个人物
        num_persons_to_process = min(2, num_persons)
        for i, person in enumerate(data["pose"][:num_persons_to_process]):
            with container.expander(f"人物 #{i + 1} 详情", expanded=False):
                keypoints = person["keypoints"]
                keypoint_scores = person["keypoint_scores"]
                valid_kps = 0
                for kp_idx, kp in enumerate(keypoints):
                    if kp_idx < len(keypoint_scores):
                        confidence = keypoint_scores[kp_idx]
                        st.write(f"人物 #{i + 1} 关键点 {kp} 置信度: {confidence}")
                        if confidence > 0.5:
                            valid_kps += 1

                relevant_points = [5, 6, 11, 12, 13, 14]
                person_key = f"person_{i}"
                for detection in data["yolo"]:
                    if detection["class_name"] in ["Operating gown", "Surgical gown"]:
                        box = detection["bbox"]
                        inside_count = 0
                        for point_idx in relevant_points:
                            if point_idx < len(keypoints):
                                point = keypoints[point_idx]
                                print(point)
                        ...
                        if inside_count >= 4:
                            if len(keypoints) >= 13:
                                ...
                                # 检测任意一只手低于某个腰
                                any_hand_low_waist_log = ""
                                any_hand_low_waist_condition = (left_hand_y > left_waist_y) or (left_hand_y > right_waist_y) or \
                                                               (right_hand_y > left_waist_y) or (right_hand_y > right_waist_y)
                                if any_hand_low_waist_condition:
                                    current_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                                    any_hand_low_waist_log = f"时间: {current_time} 人物ID: {person_key} 对应的物品: {detection['class_name']}: any hand lowed waist"

                                    # any_hand_low_waist_log = f"人物ID: {person_key} 对应的物品: {detection['class_name']}: any hand lowed waist"
                                    st.text(any_hand_low_waist_log)
                                    print(any_hand_low_waist_log)
                                    write_log_to_file(any_hand_low_waist_log)

                                if person_key not in person_any_hand_low_waist_timestamps:
                                    person_any_hand_low_waist_timestamps[person_key] = []

                                if any_hand_low_waist_condition:
                                    person_any_hand_low_waist_timestamps[person_key].append(time.time())
                                    while person_any_hand_low_waist_timestamps[person_key] and \
                                            time.time() - person_any_hand_low_waist_timestamps[person_key][0] > 2:
                                        person_any_hand_low_waist_timestamps[person_key].pop(0)

                                if len(person_any_hand_low_waist_timestamps[person_key]) >= 2 and \
                                        time.time() - person_any_hand_low_waist_timestamps[person_key][0] >= 2:
                                    print("any_hand_low_waist_log   2s")

                                # 检测任意一只手高于1或2号点
                                any_hand_above_head_log = ""
                                any_hand_above_head_condition = (left_hand_y < head1_y or left_hand_y < head2_y) or \
                                                                (right_hand_y < head1_y or right_hand_y < head2_y)
                                if any_hand_above_head_condition:
                                    any_hand_above_head_log = f"人物ID: {person_key} 对应的物品: {detection['class_name']}: any hand higher than head"
                                st.text(any_hand_above_head_log)
                                print(any_hand_above_head_log)
                                write_log_to_file(any_hand_above_head_log)

                                if person_key not in person_any_hand_high_head_timestamps:
                                    person_any_hand_high_head_timestamps[person_key] = []

                                if any_hand_above_head_condition:
                                    person_any_hand_high_head_timestamps[person_key].append(time.time())
                                    while person_any_hand_high_head_timestamps[person_key] and \
                                            time.time() - person_any_hand_high_head_timestamps[person_key][0] > 2:
                                        person_any_hand_high_head_timestamps[person_key].pop(0)

                                if len(person_any_hand_high_head_timestamps[person_key]) >= 2 and \
                                        time.time() - person_any_hand_high_head_timestamps[person_key][0] >= 2:
                                    print("any_hand_above_head_log    2s")

                st.text(f"有效关键点: {valid_kps}/{len(keypoints)}")
